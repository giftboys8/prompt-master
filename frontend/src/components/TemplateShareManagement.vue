        ...
        <div class="share-list">
          <el-table :data="shareList" style="width: 100%">
            ...
            <el-table-column
              ...
              width="180"
            />
            <el-table-column prop="can_edit" label="权限" width="100">
            ...
            <el-table-column label="操作" width="100">
              <template #default="{ row }">
                <el-button type="danger" link @click="handleRemoveShare(row)">
                  移除
                </el-button>
              </template>
            </el-table-column>
...
const handleSearch = async (query: string) => {
  ...
  try {
    ...
    userList.value = response.results.filter(
      (user) =>
        !shareList.value.some((share) => share.shared_with.id === user.id),
    );
    ...
    ElMessage.error(
      error.response?.data?.detail || error.message || "搜索用户失败",
    );
    userList.value = [];
  } finally {
    loading.value = false;
