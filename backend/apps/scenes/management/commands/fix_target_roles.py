...
class Command(BaseCommand):
    help = '修复场景目标角色数据格式，确保是标准字符串数组'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('开始修复场景目标角色数据格式...'))
        ...
        for scene in scenes:
            # 记录原始值用于比较
            original_value = scene.target_roles
            ...
            if not isinstance(scene.target_roles, list):
                try:
                    ...
                    self.stdout.write(self.style.ERROR(f"处理场景 {scene.id} 时出错: {str(e)}"))
                    scene.target_roles = []
            ...
            # 如果有变化，保存并计数
            if scene.target_roles != original_value:
                self.stdout.write(f"场景 {scene.id} ({scene.name}): {original_value} -> {scene.target_roles}")
                scene.save()
                count += 1
        
        self.stdout.write(self.style.SUCCESS(f'修复完成! 共修复 {count} 条场景记录。'))